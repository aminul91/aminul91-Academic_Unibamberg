...
router.get('/', function(req, res, next) {
  ...
  async.parallel([
      function(callback){
        ...
        rest.get('http://searchservice:8888/artists/search?artist=' + artist).on('complete', function(data) {
          console.log('Artist ID is ' + data["id"]);
          rest.get('http://chartservice:8888/charts/' + data["id"]).on('complete', function(data) {
      ...
      function(callback){
          ...
          rest.get('http://searchservice:8888/tracks/search?title=' + title + '&artist=' + artist).on('complete', function(data) {
            console.log('Title ID is ' + data["id"]);

            rest.get('http://imageservice:8888/cover-art/' + data["id"]).on('complete', function(data) {
