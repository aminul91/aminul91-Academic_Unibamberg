...
public class AlbumResource implements AlbumApi {
	...
	public List<Release> getNewReleases(@QueryParam("country") String country, @QueryParam("size") int size) throws ClientRequestException {
		...
		if(country != null) {
			...
			if(countryCode == null) {
				throw new ClientRequestException(new ErrorMessage("The provided country code does not resolve to any predefined country code"));
			} 
		}
		
		/**
		 * Api mentions that size param value must be between 01 and 50
		 */
		if(size<0 || size>50) {
